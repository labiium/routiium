...
Routiium is a self-hosted, OpenAI-compatible LLM gateway with safe-by-default routing. Put it between your apps and your model providers to add managed API keys, embedded EduRouter-style routing, request-level LLM/deterministic judging, rate limits, analytics, system prompts, MCP tools, and remote Router compatibility without changing client SDKs.
...
Your App / OpenAI SDK
  ...
  Routiium :8088 ── auth, limits, routing, judge, analytics, tools
        ...
        └── Optional EduRouter / remote Router policy service
```
...
### 3. Use built-in routing, or route through EduRouter
...
Routiium now has embedded aliases such as `auto`, `fast`, `balanced`, `safe`, `secure`, and `premium` with cost/latency/context-aware scoring. Remote router mode remains available when you want a separate EduRouter service for central policy, catalog, health, and overlay management.
...
- **Embedded EduRouter-style routing** with `auto`, `fast`, `balanced`, `safe`, `secure`, and `premium` aliases, cost/latency/context scoring, route metadata, and remote Router Schema compatibility.
